...
>「我們需要一個管理投票資料的網路程式系統，系統可以透過網路介面操作也可以透過本地的指令介面操作，系統資料包括了一些實體檔案以及資料庫。一般使用者可以透過系統觀看到個地區的投票資料，投票資料會由資料庫中取出，所以要有能夠把實體檔案的資料放入資料庫的功能。管理員能夠進入系統的控制界面，他可以上傳新的投票資料、瀏覽投票結果的檔案以及對投票的檔案進行改名。針對實體的檔案，系統還提供檔案處理功能，至少包含檔名對應、檔名處理以及檔案管理，讓管理員能夠快速地重新命名大量的檔案，並移動、分類進適當的資料夾中。但系統在設計上必須要留有能夠擴充的彈性，未來可能會新增不同的檔案處理功能。另外這套系統還會提供資料分析以及資料爬蟲這兩大功能，資料分析要能夠進行一些基本的統計功能（但需要能擴充），資料爬蟲則要能夠讓管理員選擇爬取不同網站的資料，或選擇不同的爬蟲套件來抓取資料。」

在這份作業中你需要完成三個任務，分析、設計以及實作。

## Exercise 1

請利用物件導向的概念分析你的問題，至少要描述問題中可能會有哪些物件，而這些物件會有怎麼樣的互動。

## Exercise 2

請利用物件導向的概念設計出一套系統，你需要詳細描述系統中會有哪些類別、這些類別會有什麼性質跟方法。另外你還需要描述物件互動的細節。

## Exercise 3

使用 Python 和物件導向的概念，撰寫系統的架構（例如一堆空的 classes，你不需要真的實作每一個功能，只需要一個空殼，但要有註解），請善用模組與套件，並透過 `sphinx` 產生出系統文件。

-------

這一份練習基本上不會有標準答案，只是希望透過這樣的步驟讓大家稍微理解到物件導向的程式開發是什麼樣子，這不是解決問題的唯一方法。

如果你需要一些實際的物件導向設計流程範例的話，可以繼續往下看。
...
## Step 1 - 物件導向分析 Object-Oriented Analysis
...
## Step 2 - 物件導向設計 Object-Oriented Design
...
## Step 3 - 物件導向實作 Object-Oriented Implementation
