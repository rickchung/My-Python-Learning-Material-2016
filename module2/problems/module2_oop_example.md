...
>「我們需要一個管理投票資料的網路程式系統，系統可以透過網路介面操作也可以透過本地的指令介面操作，系統資料包括了一些實體檔案以及資料庫。一般使用者可以透過系統觀看到個地區的投票資料，投票資料會由資料庫中取出。管理員能夠進入系統的控制界面，他可以上傳新的投票資料、瀏覽投票結果的檔案以及對投票的檔案進行改名。針對實體的檔案，系統還提供檔案處理功能，至少包含檔名對應、檔名處理以及檔案管理這三大功能，讓管理員能夠快速地重新命名大量的檔案，並移動、分類進適當的資料夾中。但系統在設計上必須要留有能夠擴充的彈性，未來可能會新增不同的檔案處理功能。另外這套系統還會提供資料分析以及資料爬蟲這兩大功能，資料分析要能夠進行一些基本的統計功能（但需要能擴充），資料爬蟲則要能夠讓管理員選擇爬取不同網站的資料。」
